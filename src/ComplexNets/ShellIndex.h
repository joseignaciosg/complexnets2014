...
private:
	...
	void calculateShellIndex() {
		...
		while (getNextNode() != NULL) {
			...
			while (!neighborsIt.end()) {
				...
				if (debug) {
					outfile << "	Neighbor: " << neigh->getVertexId() << " with degree: " << neighNode->currentDegree << "\n";
					outfile.flush();
				}
	...
	void initMultimapSet(Graph& g) {
	    ...
	    nodesByCurrentDegree = new Dllist[totalVertexes]; // 0 won't exist, still max degree possible is n-1
